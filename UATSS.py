...
def compute_uncertainty(model, inputs, num_samples=10):
    model.train()  # 保持Dropout开启
    outputs = [model(inputs) for _ in range(num_samples)]
    outputs = torch.stack(outputs)  # (num_samples, batch_size, num_classes, h, w)
...
class ATSSAssigner(nn.Module):
    ...
    def __init__(self,
                 topk=9,
                 num_classes=80,
                 uncertainty_weight=0.1):  # 新增不确定性权重参数
        super(ATSSAssigner, self).__init__()
        self.topk = topk
        self.num_classes = num_classes
        self.bg_idx = num_classes
        self.uncertainty_weight = uncertainty_weight  # 保存不确定性权重
    ...
    def forward(self,
        ...
        # 计算 IoU，假设使用的是 iou_calculator 函数
        ious = iou_calculator(gt_bboxes, anc_bboxes)

        # 计算 iou_threshold
        iou_threshold = 0.5 + self.uncertainty_weight * uncertainty

        # 继续处理，根据 iou_threshold 进行正负样本分配
        # 例如:
        ...
        Args:
            uncertainty (Tensor): shape(bs, h*w)
